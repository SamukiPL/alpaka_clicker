...
class PropertiesRepositoryImpl implements PropertiesRepository {
  ...
  PropertiesRepositoryImpl(this._moneyService, this._propertiesService, this._storeClerc);
  ...
  Stream<Result<List<PropertyModel>>> getPropertiesList() {
    return CombineLatestStream.combine2(
      ...
      final offers = properties.map((property) => property.getOffer(offerCount: buyAmount)).toList();
      _storeClerc.updateCurrentOffers(offers);

      final models = offers.map((offer) => PropertyModel(offer: offer, canBuy: offer.canBeBought(money))).toList();
