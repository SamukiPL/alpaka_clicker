...
import 'package:alpaka_clicker/clicker_base/money/data/money_service_impl.dart';
...
void main() {
  ...
  test("Bank payInterest gets invoked", () async {
    final underTest = MoneyServiceImpl(bank, beautifier);
  ...
  test("UnderTest returns stream with depostied Currency", () async {
    final underTest = MoneyServiceImpl(bank, beautifier);
    ...
    underTest.getDepositedMoney().listen(expectAsync1((event) {
          expect(event, expectedCurrency);
          verify(bank.getMoney()).called(1);
        }, max: 1));
    underTest.payInterest();
  });

  test("UnderTest returns stream with Result<String> and emits Success when underTest.payInterest is invoked",
      () async {
    final underTest = MoneyServiceImpl(bank, beautifier);
